...
class Px4Controller:
    ...
    def __init__(self):
        ...
        self.except_dist = 4  # 期望与岸的距离为 0.7
    ...
    def plot(self):
        while True:
            ...
            if self.line !=None:
                ...
                paths = self.paths 
                print ("self.num ------ plot",self.paths[-1][1])
...
# 设置为手动模式
    ...
    def point_cloud_callback(self, msg):
        ...
        self.cloud_points = list(pc2.read_points(msg, field_names=("x", "y", "z"), skip_nans=True))
    ...
    def get_dist_parallel_yaw(self):
        ...
        # acquired = self.point_set_mutex.acquire(True)  # blocking.  尝试获得锁定。使线程进入同步阻塞状态
        # if acquired:
        ...
        # dist/math.sin(theta)-x/math.tan(theta)
            self.line = LineSegment.segment_Trig(self.paths,center_point=center_point)
            # 当线段拟合程度不够时使用双折线拟合
        ...
        print("self.left_dist",self.left_dist)
        print("self.left_parallel_yaw",self.left_parallel_yaw)
        # print("self.num -------point_cloud",self.num)
        # print("point_time :  ",end_time-start_time)
        #     return
        # else:
...
# imu 回调函数
    def imu_callback(self, msg):
        ...
        self.yaw =  (-self.current_heading / math.pi * 180.0 +90 )%360
        # print (" yaw ",yaw)
        # print ("self. yaw ",self.yaw)

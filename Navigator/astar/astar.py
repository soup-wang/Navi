...
class A_star(object):
    def __init__(self,end_pos):
        ...
        self.openlist = []          
        self.closed = []           
        self.movement_list = astar_config.astar_config['movement_list']     # 使用一个五元方程将路线光滑
        self.func_h = astar_config.astar_config['func_h']   
        self.horiontal_radius, self.vertical_radius = self.__aircraft_radius(aircraft_obj['aircraft_points']) # 得到机身的大小  水平半径，垂直半径
    ...
    def is_valid(self, pt, obstacle_map):
        global aircraft_obj
        obstacle_map_indexed = set(obstacle_map) # 将八叉树放入到set 集合中
        aircraft_points = []
        ...
        if obstacle_map_indexed & aircraft_indexed: # 求两个set 集合的交集，若有相同的值为true ,返回值为交集的值，
            return False
        else:
            return True
